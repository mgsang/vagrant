...
## For VM Hosts instance  
  NODE_COUNT.times do |i|
    node_id = "vmhost0#{i}"
    config.vm.define node_id do |node|
    config.vm.provision "shell", path: "./provisioning/vmhosts.sh", args: ""
    config.vm.network "forwarded_port", guest: 8080, host: "880#{i}"
...
## For Ansible Server
    ...
    config.vm.define node_id do |node|
    config.vm.provision "shell", path: "./provisioning/vmhosts.sh", args: ""
    config.vm.network "forwarded_port", guest: 22, host: "40000"
    config.vm.network "private_network", :dev => "eth0", :ip => "192.200.10.5"
